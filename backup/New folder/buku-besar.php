...
if ($id_akun) {
    ...
    // Get saldo awal (saldo sebelum periode)
    $bulan_sebelum = date('Y-m-d', strtotime($bulan . '-01 -1 day'));
    
    // Calculate saldo awal from jurnal
    ...
    $query = "
        SELECT 'Debit' as posisi, tanggal, deskripsi, nominal, referensi
        FROM jurnal_umum
        WHERE id_akun_debit = $id_akun
        ...
        SELECT 'Kredit' as posisi, tanggal, deskripsi, nominal, referensi
        FROM jurnal_umum
        WHERE id_akun_kredit = $id_akun
        AND DATE_FORMAT(tanggal, '%Y-%m') = '$bulan'
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6 text-end">
                <div class="p-3 bg-light rounded">
                    <p class="mb-1 small text-muted">Saldo Awal Periode</p>
